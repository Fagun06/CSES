...
ll fun(ll n, vector<ll>&v, ll k,vector<vector<int>>&dp) {
    if(k==0) return 1;
        if(n==0) {
            return (v[0]==k);
        }
        ...
        ll notake=fun(n-1,v,k,dp);
        ll take=0;
        if(v[n]<=k) take=fun(n-1,v,k-v[n],dp);
        return dp[n][k]=(take+notake)%MOD;
    }
void solve(){
    ll n; cin>>n;
    ll sum=(n*(n+1))/2;
    ...
    cout<<(fun(n-1,v,sum/2,dp)*500000004)%MOD;
}
int main()
{
    io();
